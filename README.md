...
보습·모공·주름 정보 사이트 (정적 · GitHub Pages)

**상태: 임시 배포 (커스텀 도메인 연결 전)**

- 임시 주소: https://mgyu0904.github.io/boseup/
- 목표 도메인: `boseup.org` (DNS 미연결)

## 지금 검색 색인이 막혀 있다 — 의도된 것이다

임시 주소가 색인되면 `boseup.org` 을 붙인 뒤 같은 글이 두 주소로 잡혀 중복이 된다.
...
1. 가비아에서 `boseup.org` DNS 설정 (A 4개 + www CNAME)
2. DNS 전파 확인 후 Settings > Pages > Custom domain 에 `boseup.org` 입력
3. 재빌드 — `--staging` 플래그를 **빼고** 돌리면 색인 차단이 자동으로 풀린다

```bash
python3 build.py --site boseup          # 색인 허용 + CNAME 생성 + 실도메인 절대경로
python3 tools/deploy_pages.py --apply --prod --site boseup
```
